...
class DiffusionPipeline(BasicModel):
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx :int):
        ...
        loss = 0
        weights = [1 / 2 ** i for i in
                   range(1 + len(pred_vertical))]  # horizontal (equal) + vertical (reducing with every step down)
        ...
        if self.estimate_variance:
            ...
            kl_loss = torch.mean(kl_gaussians(true_mean, true_logvar, pred_mean, pred_logvar),
                                 dim=list(range(1, x_0.ndim)))
            nnl_loss = torch.mean(F.gaussian_nll_loss(pred_x_0, x_0, torch.exp(pred_logvar), reduction='none'),
                                  dim=list(range(1, x_0.ndim)))
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            sample_img = self.sample(num_samples=self.num_samples, img_size=x_0.shape[1:],
                                     condition=sample_cond).detach()
